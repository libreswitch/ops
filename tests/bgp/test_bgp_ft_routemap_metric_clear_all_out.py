...
def clearSoftOutAll(dut):
    ...
    devIntReturn = dut.DeviceInteract(command="do clear bgp * soft out")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute clear bgp * soft out"
    return True
...
def verify_routemap_set_metric_clear_soft_out_all(**kwargs):
    ...
    assert (set_metric_flag_1 == True), "Failure to verify clear bgp all soft" \
                                        " out command on peer 2" \
                                        " for neighbor %s" % IP_ADDR1
    LogOutput('info',"### 'clear bgp all soft out' validated succesfully " \
    ...
    assert (set_metric_flag_3 == True), "Failure to verify clear bgp all soft" \
                                        " out command on peer 2" \
...
def verify_routemap_set_metric_clear_soft_in_all(**kwargs):
    LogOutput('info',"\n\n########## Verifying route-map set metric test 1" \
    ...
    assert (set_metric_flag_3 == True), "Failure to verify clear bgp all soft" \
                                        " in command on peer 2" \
                                        " for neighbor %s" % IP_ADDR3
    LogOutput('info',"### 'clear bgp all soft in' validated succesfully " \
...
class Test_bgp_metric_clear_all_configuration:
    def setup_class(cls):
        ...
        #    Get topology object
        Test_bgp_metric_clear_all_configuration.topoObj = \
            Test_bgp_metric_clear_all_configuration.testObj.topoObjGet()

    def teardown_class(cls):
        Test_bgp_metric_clear_all_configuration.topoObj.terminate_nodes()
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric_clear_soft_out_all(switch1=dut01Obj,
                                                     switch2=dut02Obj,
                                                     switch3=dut03Obj)
