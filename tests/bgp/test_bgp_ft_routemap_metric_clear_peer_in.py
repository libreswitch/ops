...
def clearSoftInNeighbor(dut, neighbor_addr):
    if (enterConfigShell(dut) is False):
        return False
    ...
    devIntReturn = dut.DeviceInteract(command="do clear bgp "+ neighbor_addr +
                                      " soft in")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute clear bgp %s soft in" \
                         % neighbor_addr
    return True
...
class Test_bgp_metric_clear_peer_configuration:
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric_clear_soft_in_peer(switch1=dut01Obj,
                                                     switch2=dut02Obj,
                                                     switch3=dut03Obj)
