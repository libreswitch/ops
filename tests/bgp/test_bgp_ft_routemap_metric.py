...
def verify_routemap_set_metric(**kwargs):
    ...
    assert (set_metric_flag == True), "Failure to configure 'set metric' command"
    LogOutput('info',"### 'set metric' running succesfully ###\n")


def verify_routemap_set_metric_1(**kwargs):
    ...
    result=configure_route_map_set_metric(switch1,"BGP_OUT",AS_NUM1, metric)
    ...
    LogOutput('info',"Configuring no route-map on SW2")
    result=configure_no_route_map(switch2,"BGP_IN")
    assert result is True, "Failed to configure route-map on SW2"

    LogOutput('info',"Configuring route-map on SW2")
    result=configure_route_map_match_metric(switch2,"BGP_IN",AS_NUM2, metric)
    assert result is True, "Failed to configure route-map on SW2"
    ...
    assert (set_metric_flag == True), "Failure to configure 'set metric' command"
    LogOutput('info',"### 'set metric','match metric' running succesfully ###\n")

def verify_routemap_set_metric_2(**kwargs):
    ...
    assert (set_metric_flag == False), "Failure to configure 'set metric','match metric' command"
    LogOutput('info',"### 'set metric','match metric' test 2 running succesfully ###\n")
...
class Test_bgp_redistribute_configuration:
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric(switch1=dut01Obj, switch2=dut02Obj)
        verify_routemap_set_metric_1(switch1=dut01Obj, switch2=dut02Obj)
        verify_routemap_set_metric_2(switch1=dut01Obj, switch2=dut02Obj)
