...
def clearSoftInPeerGroup(dut, group):
    if (enterConfigShell(dut) is False):
        return False

    devIntReturn = dut.DeviceInteract(command="do clear bgp peer-group "
                                             + group + \
                                             " soft in")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute clear bgp peer-group %s soft in" \
                         % group
    return True
...
def verify_routemap_set_metric_clear_soft_in_peer_group(**kwargs):
    ...
    result=configure_route_map_set_metric(switch2,"BGP_IN",AS_NUM2, metric)
    ...
    LogOutput('info',"Configuring peer group")
    result = configure_peer_group(switch2, AS_NUM2,
                                           "pGroup")
    assert result is True, "Failed to configure peer group"

    LogOutput('info',"Adding neighbor 1 to peer group")
    result = configure_peer_group_member(switch2, AS_NUM2, IP_ADDR1,
                                                "pGroup")
    assert result is True, "Failed to add neighbor 1 to peer group"

    LogOutput('info',"Adding neighbor 3 to peer group")
    result = configure_peer_group_member(switch2, AS_NUM2, IP_ADDR3,
                                           "pGroup")
    assert result is True, "Failed to add neighbor 3 to peer group"

    LogOutput('info',"Configuring neighbor route-map on SW2 for peer group")
    result = configure_neighbor_rmap_in(switch2, AS_NUM2, "pGroup", AS_NUM1,
                                         "BGP_IN")
    assert result is True, "Failed to configure neighbor route-map on SW2" \
                            "for peer group"
    clearSoftInPeerGroup(switch2, "pGroup")
    ...
    assert (set_metric_flag_1 == True), "Failure to verify clear bgp peer group" \
                                        " soft in command on peer 2 for " \
                                        "neighbor %s" % IP_ADDR1
    assert (set_metric_flag_3 == True), "Failure to verify clear bgp peer group" \
                                        " soft in command on peer 2 for" \
                                        " neighbor %s" % IP_ADDR3
    LogOutput('info',"### 'clear bgp peer group soft in' validated succesfully " \
...
class Test_bgp_metric_clear_peer_group_configuration:
    def setup_class(cls):
        Test_bgp_metric_clear_peer_group_configuration.testObj = \
            testEnviron(topoDict=topoDict)
        #    Get topology object
        Test_bgp_metric_clear_peer_group_configuration.topoObj = \
            Test_bgp_metric_clear_peer_group_configuration.testObj.topoObjGet()

    def teardown_class(cls):
        Test_bgp_metric_clear_peer_group_configuration.topoObj.terminate_nodes()
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric_clear_soft_in_peer_group(switch1=dut01Obj,
                                                     switch2=dut02Obj,
                                                     switch3=dut03Obj)
