...
def clearSoftInAsn(dut, asn):
    ...
    devIntReturn = dut.DeviceInteract(command="do clear bgp "+ asn +
                                      " soft in")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute clear bgp %s soft in" % asn
    return True
...
def verify_routemap_set_metric_clear_soft_in_as(**kwargs):
    ...
    assert result is True, "Failed to configure neighbor route-map on SW2" \
                           "for peer 3"
    clearSoftInAsn(switch2, AS_NUM1)
    clearSoftInAsn(switch2, AS_NUM3)
    ...
    LogOutput('info',"### 'clear bgp as soft in' validated succesfully " \
...
class Test_bgp_metric_clear_as_configuration:
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric_clear_soft_in_as(switch1=dut01Obj,
                                                     switch2=dut02Obj,
                                                     switch3=dut03Obj)
