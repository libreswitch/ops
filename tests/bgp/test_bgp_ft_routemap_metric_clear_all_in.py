...
def clearSoftInAll(dut):
    if (enterConfigShell(dut) is False):
        return False

    devIntReturn = dut.DeviceInteract(command="do clear bgp * soft in")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute clear bgp * soft in"
    return True
...
class Test_bgp_metric_clear_all_configuration:
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric_clear_soft_in_all(switch1=dut01Obj,
                                                     switch2=dut02Obj,
                                                     switch3=dut03Obj)
