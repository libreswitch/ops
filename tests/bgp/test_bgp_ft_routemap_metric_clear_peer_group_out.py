...
def clearSoftOutPeerGroup(dut, group):
    if (enterConfigShell(dut) is False):
        return False
    ...
    devIntReturn = dut.DeviceInteract(command="do clear bgp peer-group "
                                              ...
                                              " soft out")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute clear bgp peer-group %s soft out" \
                         % group
    return True
...
def verify_routemap_set_metric_clear_soft_out_peer_group(**kwargs):
    ...
    result = configure_neighbor_rmap_out(switch2, AS_NUM2, "pGroup", AS_NUM1,
                                         "BGP_OUT")
    assert result is True, "Failed to configure neighbor route-map on SW2" \
                            "for peer group"

    clearSoftOutPeerGroup(switch2, "pGroup")
    ...
    LogOutput('info',"### 'clear bgp peer group soft out' validated succesfully" \
...
class Test_bgp_metric_clear_peer_group_configuration:
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric_clear_soft_out_peer_group(switch1=dut01Obj,
                                                     switch2=dut02Obj,
                                                     switch3=dut03Obj)
