...
def clearSoftOutNeighbor(dut, neighbor_addr):
    if (enterConfigShell(dut) is False):
        return False

    devIntReturn = dut.DeviceInteract(command="do clear bgp "+ neighbor_addr +
                                      " soft out")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute clear bgp %s soft out" \
                         % neighbor_addr
    return True
...
def verify_routemap_set_metric_clear_soft_out_peer(**kwargs):
    ...
    clearSoftOutNeighbor(switch2, IP_ADDR1)
    clearSoftOutNeighbor(switch2, IP_ADDR3)
    ...
    assert (set_metric_flag_1 == True), "Failure to verify clear bgp peer soft" \
                                        " out command on peer 2" \
                                        " for neighbor %s" % IP_ADDR1
    LogOutput('info',"### 'clear bgp peer soft out' validated succesfully " \
    ...
    assert (set_metric_flag_3 == True), "Failure to verify clear bgp peer soft" \
                                        " out command on peer 2" \
...
def verify_routemap_set_metric_clear_soft_in_peer(**kwargs):
    LogOutput('info',"\n\n########## Verifying route-map set metric test 1" \
    ...
    LogOutput('info',"Configuring neighbor route-map on SW2 for peer 1")
    result = configure_neighbor_rmap_in(switch2, AS_NUM2, IP_ADDR1, AS_NUM1,
                                         "BGP_IN")
    assert result is True, "Failed to configure neighbor route-map on SW2" \
                           "for peer 1"

    LogOutput('info',"Configuring neighbor route-map on SW2 for peer 3")
    result = configure_neighbor_rmap_in(switch2, AS_NUM2, IP_ADDR3, AS_NUM3,
                                         "BGP_IN")
    assert result is True, "Failed to configure neighbor route-map on SW2" \
                           "for peer 3"

    clearSoftInNeighbor(switch2, IP_ADDR1)
    clearSoftInNeighbor(switch2, IP_ADDR3)
    ...
    assert (set_metric_flag_3 == True), "Failure to verify clear bgp peer soft" \
                                        " in command on peer 2" \
                                        " for neighbor %s" % IP_ADDR3
    LogOutput('info',"### 'clear bgp peer soft in' validated succesfully " \
...
class Test_bgp_metric_clear_peer_configuration:
    def setup_class(cls):
        Test_bgp_metric_clear_peer_configuration.testObj = \
            testEnviron(topoDict=topoDict)
        #    Get topology object
        Test_bgp_metric_clear_peer_configuration.topoObj = \
            Test_bgp_metric_clear_peer_configuration.testObj.topoObjGet()

    def teardown_class(cls):
        Test_bgp_metric_clear_peer_configuration.topoObj.terminate_nodes()
    ...
    def test_configure(self):
        ...
        verify_routemap_set_metric_clear_soft_out_peer(switch1=dut01Obj,
                                                     switch2=dut02Obj,
                                                     switch3=dut03Obj)
