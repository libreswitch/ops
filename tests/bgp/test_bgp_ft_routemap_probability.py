...
def verify_routemap_match_probability(**kwargs):
    ...
    LogOutput('info',"Configuring networks on SW1")
    result = configure_network(switch1,AS_NUM1,"12.0.0.0/8")
    assert result is True, "Failed to configure network on SW1"
    LogOutput('info',"Configuring networks on SW1")
    result = configure_network(switch1,AS_NUM1,"13.0.0.0/8")
    assert result is True, "Failed to configure network on SW1"
    LogOutput('info',"Configuring networks on SW1")
    result = configure_network(switch1,AS_NUM1,"14.0.0.0/8")
    assert result is True, "Failed to configure network on SW1"
    LogOutput('info',"Configuring networks on SW1")
    result = configure_network(switch1,AS_NUM1,"15.0.0.0/8")
    assert result is True, "Failed to configure network on SW1"
    LogOutput('info',"Configuring networks on SW1")
    result = configure_network(switch1,AS_NUM1,"16.0.0.0/8")
    assert result is True, "Failed to configure network on SW1"
    ...
    result=configure_route_map_match_probability(switch2,"BGP_IN2",AS_NUM2,probability)
    ...
    result = configure_neighbor_rmap_in(switch2, AS_NUM2, IP_ADDR1, AS_NUM1, "BGP_IN2")
    ...
    assert (match_prob_flag == True) , "Failed to configure 'match probability'"

    LogOutput('info',"### 'match probability' running succesfully ###\n")
...
class Test_bgp_redistribute_configuration:
    ...
    def test_configure(self):
        ...
        verify_routemap_match_probability(switch1=dut01Obj, switch2=dut02Obj)
