...
def verify_routemap_set_aspath_prepend(**kwargs):
    ...
    result=configure_route_map_set_aspath(switch1,"BGP_OUT",AS_NUM1)
    ...
    lines = dump.split('\n')
    for line in lines:
        if neighbor_network in line and set_aspath_str in line and  network_p1 in line:
            set_aspath_flag = True
    assert (set_aspath_flag == True), "Failed to configure set as-path "
    info("### 'set as-path prepend' running succesfully ###\n")

def verify_routemap_match_aspath(**kwargs):
    ...
    result=configure_route_map_match_aspath(switch2,"BGP_IN",AS_NUM1)
    ...
    LogOutput('info',"Configuring neighbors on SW1")
    result = configure_neighbor(switch2,AS_NUM2,IP_ADDR1,AS_NUM1)
    assert result is True, "Failed to configure neighbor on SW2"
    ...
    lines = dump.split('\n')
    for line in lines:
        if neighbor_network in line and match_aspath_str in line and  network_p1 in line:
            match_aspath_flag = True
    assert (match_aspath_flag == True), "Failed to configure match as-path "
...
class Test_bgp_redistribute_configuration:
    ...
    def test_configure(self):
        ...
        verify_routemap_set_aspath_prepend(switch1=dut01Obj, switch2=dut02Obj)
        verify_routemap_match_aspath(switch1=dut01Obj, switch2=dut02Obj)
