...
def restTestRoutemaps(wrkston01):
    ...
    "GET for url=/rest/v1/system/route_maps"'
    ...
    assert retStruct.data['http_retcode'].find(
        '200') != -1, 'Rest GET route_maps Failed\n' +\
    ...
    assert retStruct.data["response_body"].find(
        '[]') != -1, 'Failed in checking the GET METHOD JSON\
        response validation for route_maps'
...
class Test_ft_framework_rest:
    ...
    def test_restTestRoutemaps(self):
        info('#######################################################\n')
        ...
        info('### Successful in test rest Routemaps ###\n')
