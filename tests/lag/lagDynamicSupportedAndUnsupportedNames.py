...
def configureLag(dut01Obj, lagName, negative):
    ...
    if retStruct.returnCode() != 0:
        ...
        LogOutput('info', "LAG  " + str(lagName) + " successfully created")

    LogOutput('info', "############################################")
    LogOutput('info', "Configure LAG Mode using name: " + str(lagName))
    LogOutput('info', "############################################")

    retStruct = lagMode(deviceObj=dut01Obj, lagId=lagName, lacpMode="active")

    if negative:
        if retStruct.returnCode() == 0 and retCode == 0:
            LogOutput('error', "LAG with invalid name " +
                      str(lagName) + " successfully configured")
            return False
        else:
            LogOutput('info', "LAG with invalid name " +
                      str(lagName) + " was not configured, as expected")
            return True
    else:
        if retStruct.returnCode() != 0 or retCode != 0:
            LogOutput(
                'error', "Failed to configure LAG with name " + str(lagName))
            return False
        else:
            LogOutput('info', "Successfully configured LAG with name \
                " + str(lagName))
            return True
...
class Test_lagDynamicSupportedAndUnsupportedNames:
    ...
    def setup_class(cls):
        ...
        Test_lagDynamicSupportedAndUnsupportedNames.testObj = testEnviron(
            topoDict=topoDict)
        Test_lagDynamicSupportedAndUnsupportedNames.topoObj = \
            Test_lagDynamicSupportedAndUnsupportedNames.testObj.topoObjGet()
        # Global variables
        global dut01Obj
