...
#              middle of the VLAN list or the VLAN not with the highest
#              or lowest VID. No traffic should pass through other vlans.
#
...
class Test_vlan_state_removed_from_middle_of_table:
    ...
    def test_enable_vlan(self):
        ...
        # Enable vlan 3
        if VlanStatus(deviceObj=self.dut01Obj, vlanId=3,
                      status=True).returnCode() != 0:
        ...
        # Enable vlan 4
        if VlanStatus(deviceObj=self.dut01Obj, vlanId=4,
                      status=True).returnCode() != 0:
    ...
    def test_addPort_vlans(self):
        ...
        returnStr = AddPortToVlan(
            deviceObj=self.dut01Obj, vlanId=2, interface=2,
        ...
        returnStr = AddPortToVlan(
            deviceObj=self.dut01Obj, vlanId=3, interface=3,
            access=True, allowed=False, tag=False, config=True)
        if returnStr.returnCode() != 0:
        ...
        returnStr = AddPortToVlan(
            deviceObj=self.dut01Obj, vlanId=4, interface=4,
            access=True, allowed=False, tag=False, config=True)
        if returnStr.returnCode() != 0:
    ...
    def test_delete_middle_vlan(self):
        LogOutput('info', "############################################")
        LogOutput(
            'info', "Step 5- Delete middle vlan value of VID from table")
        LogOutput('info', "############################################")
        # Delete middle vlan in table
        if AddVlan(deviceObj=self.dut01Obj, vlanId=3,
                   config=False).returnCode() != 0:
            LogOutput('error', "Failed to delete vlan " + str(3))
            assert(False)
        else:
    ...
    def test_ReAssign_Port(self):
        ...
        # Add port 2 to vlan 2
        returnStr = AddPortToVlan(
            deviceObj=self.dut01Obj, vlanId=2, interface=3, access=True,
