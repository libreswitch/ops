...
def sub_interface(**kwargs):
    ...
    retStruct = Dot1qEncapsulation(deviceObj=device1, subInterface="1.2",
                                   dot1q=True, vlan=12)
    retCmdout = wrkstn1.DeviceInteract(command=
                "ip link add link wrkston01-eth0 \
                         name host01-eth0.12 type vlan id 12")
    retCmdout = wrkstn1.DeviceInteract(command=
                "ip link set dev host01-eth0.12 up")
    retCmdout = wrkstn1.DeviceInteract(command=
                "ip -6 address add 2000::9/64 dev host01-eth0.12")
    sleep(2)
    retStruct = wrkstn1.Ping(ipAddr="2000::23", ipv6Flag=True, packetCoung=10)
    retCode = retStruct.returnCode()
    assert retCode == 0, "failed to ping switch"
    LogOutput('info', "IPv4 Ping from workstation 1 to dut01 return \
                       JSON:\n" + str(retStruct.retValueString()))
...
class Test_subInt:
    def setup_class(cls):
        ...
        if Test_subInt.topoObj.topoType == "physical":
            LogOutput('info',
