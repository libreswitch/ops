...
def fastpath_ping(**kwargs):
    ...
    assert retCode == 0, "Unable to enable interafce on SW1"
    ...
    retCode = retStruct.returnCode()
    assert retCode == 0, "Unable to enable interafce on SW2"
    ...
    retCode = retStruct.returnCode()
    assert retCode == 0, "Failed to configure ipv4 address route"
    ...
    retCode = retStruct.returnCode()
    assert retCode == 0, "Failed to configure ipv6 address route"
    ...
    retCode = retStruct['returnCode']
    assert retCode == 0, "couldn't get l3route debug"
    ...
    assert routerow, "route not programmed in ASIC on device1"
    columns = routerow.split()
    routehit = columns[5]
    assert routehit == 'Y', "route not selected in ASIC on device1"
    ...
    assert routerow, "route not programmed in ASIC on device2"
    columns = routerow.split()
    routehit = columns[5]
    assert routehit == 'Y', "route not selected in ASIC on device2"
    ...
    retCode = retStruct.returnCode()
    assert retCode == 0, "Failed to unconfigure ipv4 address route"
    ...
    retCode = retStruct.returnCode()
    assert retCode == 0, "Failed to unconfigure ipv6 address route"
    ...
    retCode = retStruct.returnCode()
    assert retCode, "##### Ping succeeded even after unconfiguring routes #####"
    ...
    retStruct = device4.Ping(ipAddr="10.0.10.1", packetCount=1)
    retCode = retStruct.returnCode()
    assert retCode, "##### ping succeeded even after unconfigureing routes #####"
...
class Test_fastpath_ping:
    ...
    def test_fastpath_ping(self):
        ...
        fastpath_ping(device1=dut01Obj, device2=dut02Obj, device3=wrkston01Obj, device4=wrkston02Obj)
